...
def build_and_run(seed: int = 0, enable_softmax: bool = True, use_dynamic_quant: bool = False):
    ...
    b_fc0 = rng.integers(-128, 128, size=(ff_dim,), dtype=np.int8)
    dense0 = DenseLayer(name='dense_0', weight=W_fc0, bias=b_fc0, shift=15, scale=115, relu=True)
    ...
    Bo = rng.integers(-128, 128, size=(ff_dim,), dtype=np.int8)
    ...
    b_ff1a = rng.integers(-128, 128, size=(ff_dim,), dtype=np.int8)
    ff1a = DenseLayer(name='ff1a', weight=W_ff1a, bias=b_ff1a, shift=15, scale=42, relu=True)
    model.add_layer(ff1a, inputs=[res1])

    W_ff1b = rng.integers(-128, 128, size=(ff_dim, ff_dim), dtype=np.int8)
    b_ff1b = rng.integers(-128, 128, size=(ff_dim,), dtype=np.int8)
    ff1b = DenseLayer(name='ff1b', weight=W_ff1b, bias=b_ff1b, shift=15, scale=91, relu=True)
    ...
    b_ff2a = rng.integers(-128, 128, size=(ff_dim,), dtype=np.int8)
    ff2a = DenseLayer(name='ff2a', weight=W_ff2a, bias=b_ff2a, shift=15, scale=31, relu=True)
    model.add_layer(ff2a, inputs=[res3])

    W_ff2b = rng.integers(-128, 128, size=(ff_dim, ff_dim), dtype=np.int8)
    b_ff2b = rng.integers(-128, 128, size=(ff_dim,), dtype=np.int8)
    ff2b = DenseLayer(name='ff2b', weight=W_ff2b, bias=b_ff2b, shift=15, scale=75, relu=True)
    ...
    b_out1 = rng.integers(-128, 128, size=(ff_dim,), dtype=np.int8)
    out1 = DenseLayer(name='out1', weight=W_out1, bias=b_out1, shift=15, scale=30, relu=True)
    model.add_layer(out1, inputs=[res4])

    out_dim = 8
    W_out2 = rng.integers(-128, 128, size=(ff_dim, out_dim), dtype=np.int8)
    b_out2 = rng.integers(-128, 128, size=(out_dim,), dtype=np.int8)
    out2 = DenseLayer(name='out2', weight=W_out2, bias=b_out2, shift=15, scale=106, relu=False)
