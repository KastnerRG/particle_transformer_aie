...
def build_and_run(seed: int = 0, use_dynamic_quant: bool = True):
    ...
    dense0 = DenseLayer(name='dense_0', weight=W_fc0, bias=None, relu=True)
    model.add_layer(dense0, inputs=[None])
    ...
    ff1a = DenseLayer(name='ff1a', weight=W_ff1a, bias=None, relu=True)
    model.add_layer(ff1a, inputs=[res1])

    W_ff1b = rng.integers(-128, 128, size=(ff_dim, ff_dim), dtype=np.int8)
    ff1b = DenseLayer(name='ff1b', weight=W_ff1b, bias=None, relu=True)
    ...
    ff2a = DenseLayer(name='ff2a', weight=W_ff2a, bias=None, relu=True)
    model.add_layer(ff2a, inputs=[res3])

    W_ff2b = rng.integers(-128, 128, size=(ff_dim, ff_dim), dtype=np.int8)
    ff2b = DenseLayer(name='ff2b', weight=W_ff2b, bias=None, relu=True)
    ...
    out1 = DenseLayer(name='out1', weight=W_out1, bias=None, relu=True)
    model.add_layer(out1, inputs=[res4])

    out_dim = 8
    W_out2 = rng.integers(-128, 128, size=(ff_dim, out_dim), dtype=np.int8)
    out2 = DenseLayer(name='out2', weight=W_out2, bias=None, relu=False)
