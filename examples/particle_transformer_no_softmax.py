...
def build_and_run(seed: int = 0):
    ...
    m, k, n = 4, 8, 8
    model = AIEModel(m=m, k=k, n=n, iterations=1, dynamic_quant=True)

    W_fc0 = rng.integers(-128, 128, size=(num_feature_pad, ff_dim), dtype=np.int8)
    b_fc0 = rng.integers(-128, 128, size=(ff_dim,), dtype=np.int8)
    dense0 = DenseLayer(name='dense_0', weight=W_fc0, bias=b_fc0, relu=True)
    ...
    ff1a = DenseLayer(name='ff1a', weight=W_ff1a, bias=b_ff1a, relu=True)
    ...
    ff1b = DenseLayer(name='ff1b', weight=W_ff1b, bias=b_ff1b, relu=True)
    ...
    ff2a = DenseLayer(name='ff2a', weight=W_ff2a, bias=b_ff2a, relu=True)
    ...
    ff2b = DenseLayer(name='ff2b', weight=W_ff2b, bias=b_ff2b, relu=True)
    ...
    out1 = DenseLayer(name='out1', weight=W_out1, bias=b_out1, relu=True)
    ...
    out2 = DenseLayer(name='out2', weight=W_out2, bias=b_out2, relu=False)
